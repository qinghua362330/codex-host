...
API Key 只会以脱敏状态显示；保存或导入后，CodexHost 会热替换对应 Harness 的适配器，新建 Harness 会话立即使用新配置，已经运行中的会话继续使用原进程。无需重启 ChatGPT 或 codexhost。未检测到本机配置时，页面会显示“未检测到本机 Harness 配置”。
